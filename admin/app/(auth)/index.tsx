...
export default function AdminAuth() {
  ...
  const handleGetOtp = async () => {
    ...
    try {
      ...
      const response = await apiClient.post("/api/user/onboarding", payload);
      if (response?.data?.success) {
        await store.set("auth_phone", payload.phone);
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={styles.card}>
        ...
        <TouchableOpacity onPress={handleGetOtp} activeOpacity={0.8} style={[styles.button, (!isValidPhone || submitting) && styles.buttonDisabled]} disabled={!isValidPhone || submitting}>
          {submitting ? <ActivityIndicator color="#ffffff" /> : <Text style={styles.buttonText}>Get OTP</Text>}
        </TouchableOpacity>
      </View>
