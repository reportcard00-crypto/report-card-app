...
export default function TeacherClassroomsScreen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: "View Details", onPress: () => router.push(`/teacher/classroom-details?id=${resp.data._id}` as any) },
  ...
  const handleViewDetails = (classroom: ClassroomListItem) => {
    router.push(`/teacher/classroom-details?id=${classroom._id}` as any);
  };
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchClassrooms(false); }} colors={["#2563eb"]} />
      }
    >
